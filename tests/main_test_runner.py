...
from tests_helpers import enable_debug, logger
...
student_submissions_path = os.path.dirname(os.path.abspath(__file__))+ "/../src/"
...
if __name__ == "__main__":
  ...
  if len(sys.argv) <= 1:
    begin_testing(entire_class=True)
  else:
    enable_debug()
    begin_testing(entire_class=False, student=sys.argv[1])
